...
public final class SessionDataManager {
  ...
  public void onRegistry(ClientboundRegistryDataPacket packet) {
    ...
    /*
    NbtMap dimensionRegistry = registry.get("minecraft:dimension_type");
    for (var dimension : dimensionRegistry.getList("value", NbtType.COMPOUND)) {
      var name = dimension.getString("name");
      var id = dimension.getInt("id");

      levels.put(id, new Level(tagsState, name, id, dimension.get("element")));
    }
  ...
  public void onServerPlayData(ClientboundServerDataPacket packet) {
    serverPlayData = new ServerPlayData(packet.getMotd(), packet.getIconBytes());
  }
  ...
  public @NotNull Level currentLevel() {
    return Objects.requireNonNull(levels.get(currentDimension.dimensionId()));
  }
