...
public record MinecraftGraph(TagsState tagsState,
  ...
  private static final Object2ObjectFunction<
    ? super SFVec3i, ? extends ObjectList<GeneralBlockSubscription>>
  ...
  private static final MovementSubscription<?>[][] SUBSCRIPTION_VALUES;

  static {
    var blockSubscribers = new Vec2ObjectOpenHashMap<SFVec3i, ObjectList<GeneralBlockSubscription>>();
    ...
    SUBSCRIPTION_VALUES = new GeneralBlockSubscription[blockSubscribers.size()][];
    ...
    for (var i = 0; i < entrySetDescending.size(); i++) {
      ...
      SUBSCRIPTION_VALUES[i] = entry.getValue().toArray(new GeneralBlockSubscription[0]);
    }
  }

  private static SimpleMovement registerMovement(
    Object2ObjectMap<SFVec3i, ObjectList<GeneralBlockSubscription>> blockSubscribers,
    ...
    int movementIndex) {
    {
      ...
      for (var freeBlock : movement.requiredFreeBlocks()) {
        ...
        blockSubscribers
          ...
          .add(new GeneralBlockSubscription(movementIndex, SubscriptionType.MOVEMENT_FREE, blockId++, freeBlock.value()));
      }
    }
...

    {
      ...
      blockSubscribers
        ...
        .add(new GeneralBlockSubscription(movementIndex, SubscriptionType.MOVEMENT_SOLID));
    }
  ...
  private static ParkourMovement registerParkourMovement(
...

    {
      ...
      blockSubscribers
        ...
        .add(new GeneralBlockSubscription(movementIndex, SubscriptionType.PARKOUR_UNSAFE_TO_STAND_ON));
    }
  ...
  private static DownMovement registerDownMovement(
    ...
    int movementIndex) {
    {
      for (var safetyBlock : movement.listSafetyCheckBlocks()) {
        ...
        blockSubscribers
          ...
          .add(new GeneralBlockSubscription(movementIndex, SubscriptionType.DOWN_SAFETY_CHECK));
      }
    }

    {
      ...
      blockSubscribers
        .computeIfAbsent(freeBlock.key(), CREATE_MISSING_FUNCTION)
        .add(new GeneralBlockSubscription(movementIndex, SubscriptionType.MOVEMENT_FREE, 0, freeBlock.value()));
    }
  ...
  private void processSubscription(
    ...
    for (var subscriber : value) {
      ...
      switch (subscriber.processBlockUnsafe(this, key, action, isFreeReference, blockState, absolutePositionBlock)) {
        case CONTINUE -> {
  ...
  private record GeneralBlockSubscription(
    int actionIndex,
    SubscriptionType type,
    int blockArrayIndex,
    BlockFace blockBreakSideHint,
    BotActionManager.BlockPlaceAgainstData blockToPlaceAgainst,
    BlockSafetyData.BlockSafetyType safetyType) implements MovementSubscription<GraphAction> {
    GeneralBlockSubscription(int movementIndex, SubscriptionType type) {
      this(movementIndex, type, -1, null, null, null);
    }

    GeneralBlockSubscription(int movementIndex, SubscriptionType type, int blockArrayIndex, BlockFace blockBreakSideHint) {
      this(movementIndex, type, blockArrayIndex, blockBreakSideHint, null, null);
    }

    GeneralBlockSubscription(
      int movementIndex,
      SubscriptionType type,
      BotActionManager.BlockPlaceAgainstData blockToPlaceAgainst) {
    ...
    @Override
    public SubscriptionSingleResult processBlock(MinecraftGraph graph, SFVec3i key, GraphAction action, ObjectReference<TriState> isFreeReference,
                                                 BlockState blockState, SFVec3i absolutePositionBlock) {
      return switch (action) {
        case SimpleMovement simpleMovement -> processMovementSubscription(graph, isFreeReference, blockState, absolutePositionBlock, simpleMovement);
        case ParkourMovement ignored -> processParkourSubscription(isFreeReference, blockState);
        case DownMovement downMovement -> processDownSubscription(graph, key, blockState, absolutePositionBlock, downMovement);
        case UpMovement upMovement -> processUpSubscription(graph, isFreeReference, blockState, absolutePositionBlock, upMovement);
      };
    }

    @Override
    public GraphAction castAction(GraphAction action) {
      return action;
    }
  ...
  private interface MovementSubscription<M extends GraphAction> {
