...
public class EncryptionUtils {
  ...
  public static byte[] longToBigEndianByteArray(long... bits) {
    var ret = ByteBuffer.allocate(8 * bits.length).order(ByteOrder.BIG_ENDIAN);
    for (var put : bits) {
      ret.putLong(put);
    }
    return ret.array();
  ...
  public static byte[] parsePemEncoded(String toParse, Pair<String, String> descriptors) {
    var startIdx = toParse.indexOf(descriptors.first());
    Preconditions.checkArgument(startIdx >= 0);
    var firstLen = descriptors.first().length();
    var endIdx = toParse.indexOf(descriptors.second(), firstLen + startIdx) + 1;
