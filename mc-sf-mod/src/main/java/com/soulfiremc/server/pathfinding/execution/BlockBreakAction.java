...
public final class BlockBreakAction implements WorldAction {
  ...
  private final Direction blockBreakSideHint;
  ...
  public boolean isCompleted(BotConnection connection) {
    ...
    return SFBlockHelpers.isEmptyBlock(level.getBlockState(blockPosition));
  }

  @Override
  public BlockPos targetPosition(BotConnection connection) {
    return BlockPos.fromDouble(connection.dataManager().localPlayer().pos());
  }
  ...
  public void tick(BotConnection connection) {
    ...
    if (remainingTicks == -1) {
      var optionalBlock = level.getBlockState(blockPosition).blockType();
