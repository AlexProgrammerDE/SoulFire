...
public record MinecraftGraph(TagsState tagsState,
  ...
  private static final BlockPos[] SUBSCRIPTION_KEYS;
  private static final WrappedActionSubscription[][] SUBSCRIPTION_VALUES;

  static {
    var blockSubscribers = new Object2ObjectOpenHashMap<BlockPos, List<WrappedActionSubscription>>();
    ...
    SUBSCRIPTION_KEYS = new BlockPos[blockSubscribers.size()];
  ...
  public boolean disallowedToPlaceBlock(BlockPos position) {
    return !pathConstraint.canPlaceBlockPos(position);
  }

  public boolean disallowedToBreakBlock(BlockPos position) {
  ...
  public void insertActions(BlockPos node, @Nullable ActionDirection fromDirection, Consumer<GraphInstructions> callback) {
  ...
  private void processSubscription(
    ...
    BlockPos absolutePositionBlock = null;
  ...
  public interface MovementSubscription<M extends GraphAction> {
    ...
    default SubscriptionSingleResult processBlockUnsafe(
      ...
      GraphAction action,
      BlockState blockState,
      BlockPos absolutePositionBlock) {
