...
public final class FollowEntityController {
  ...
  public void start(BotConnection bot) {
    while (true) {
      var entity = bot.minecraft().level
      ...
      try {
        PathExecutor.executePathfinding(bot, (DynamicGoalScorer) () -> new CloseToPosGoal(BlockPos.fromInt(entity.get().blockPos()), maxRadius), new PathConstraint(bot)).get();
      } catch (Exception e) {
