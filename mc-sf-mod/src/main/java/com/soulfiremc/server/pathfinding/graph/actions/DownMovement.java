...
public final class DownMovement extends GraphAction implements Cloneable {
  ...
  private final BlockPos targetToMineBlock;
  // Mutable
  ...
  private DownMovement(SubscriptionConsumer blockSubscribers) {
    ...
    this.targetToMineBlock = FEET_POSITION_RELATIVE_BLOCK.offset(0, -1, 0);
  ...
  private void registerSafetyCheckBlocks(SubscriptionConsumer blockSubscribers) {
    // Falls one block
    blockSubscribers.subscribe(FEET_POSITION_RELATIVE_BLOCK.offset(0, -2, 0), DownSafetyCheckSubscription.INSTANCE);

    // Falls two blocks
    blockSubscribers.subscribe(FEET_POSITION_RELATIVE_BLOCK.offset(0, -3, 0), DownSafetyCheckSubscription.INSTANCE);

    // Falls three blocks
    blockSubscribers.subscribe(FEET_POSITION_RELATIVE_BLOCK.offset(0, -4, 0), DownSafetyCheckSubscription.INSTANCE);
  }
  ...
  private void registerObstructFallCheckBlocks(SubscriptionConsumer blockSubscribers) {
    ...
    blockSubscribers.subscribe(FEET_POSITION_RELATIVE_BLOCK.offset(0, -2, 0), ObstructingFallCheckSubscription.INSTANCE);

    // Block below the block we mine can obstruct a 3 block fall
    blockSubscribers.subscribe(FEET_POSITION_RELATIVE_BLOCK.offset(0, -3, 0), ObstructingFallCheckSubscription.INSTANCE);
  }

  private void registerBlockToBreak(SubscriptionConsumer blockSubscribers) {
    blockSubscribers.subscribe(targetToMineBlock, new MovementFreeSubscription(Direction.UP));
  }
  ...
  private record MovementFreeSubscription(Direction blockBreakSideHint) implements DownMovementSubscription {
    @Override
    public MinecraftGraph.SubscriptionSingleResult processBlock(MinecraftGraph graph, BlockPos key, DownMovement downMovement,
