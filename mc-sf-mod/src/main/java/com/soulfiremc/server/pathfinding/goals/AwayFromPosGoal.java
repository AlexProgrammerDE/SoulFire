 ...
 */
public record AwayFromPosGoal(BlockPos origin, int minRadius) implements GoalScorer {
  @Override
  public double computeScore(MinecraftGraph graph, BlockPos blockPosition, List<WorldAction> actions) {
    return Math.max(0, Math.sqrt(minRadius * minRadius - blockPosition.distSqr(origin)));
  }

  @Override
  public boolean isFinished(MinecraftRouteNode current) {
    return current.node().blockPosition().distSqr(origin) >= minRadius * minRadius;
  }
}
