...
public final class Costs {
  ...
  public static @Nullable BlockMiningCosts calculateBlockBreakCost(
    ...
    for (var slot : inventory.usableToolsAndEmpty()) {
      var miningTicks = getRequiredMiningTicks(blockState, player, level, pos);
  ...
  public static TickResult getRequiredMiningTicks(
    ...
    var damage = blockState.getDestroyProgress(player, level, pos);

    var willDropUsableBlockItem = player.hasCorrectToolForDrops(blockState)
      && !player.preventsBlockDrops()
      && SFBlockHelpers.isUsableBlockItem(blockType);
